...
#===============================================================================
#
#         FILE: twotier_ptdynamic.pl
#
#        USAGE: ./twotier_ptdynamic.pl  ../config/config.pm  emotion mixNumOfPhrase mixNumOfTone mode
...
if ($extract1)
{
    for my $dir (@pdir)
    {
        ...
        while (<PB>)
        {
			...
			open F0_ALL,">$dir/dct/$method/$base.f0all" or die "can't open $!\n";
			print F0_ALL"$_\n" for(@norm_f0data);
			close(F0_ALL);
		   ...
		   open NUM,">$dir/dct/$method/$base.num" or die "can't open $!\n";
            ...
            for (@words)
            {
                if (/\/p/ or /\/w/)
                {
					...
					my $wordsInPhrase = "$startTime[0] $endTime[1] ";			
                    for ($pp .. $cp)
                    {
						#print PHRASE "$words[$_]";
						$wordsInPhrase .=$words[$_];
                        print PHRASE_F0 "$words[$_]";
                    }
					push @alllines,$wordsInPhrase;
					push @alldct,@dctOfCurPhrase;
                    print PHRASE_F0 "\n";
			...
			my $T=($#alldct+1)/$dctNumOfPhrase;
			my $dim=$dctNumOfPhrase;
			my @win=(-0.5,0,0.5);
			my @alldct_d;
			for(my $t=0;$t<$T;$t++)
			{
...
if ($doTrain)
{
	...
	my $vectorLength=4*$dctNumOfPhrase;
...
if($doResynth)
{
	...
	for my $testPhraseFile(@testPhraseFiles)
	{
		...
		while(<PHRASE>)
		{
			...
			my $text=shift @phraseParas;
			@alltext=(@alltext,$text);
			@allphrase=(@allphrase,@phraseParas);
		    $T++;
=a
			print M "fprintf(fd,\'$text\\n\');\n";
			print M "x=[@phraseParas]';\n";
			my $vectorLength=$dctNumOfPhrase*4;
			print M "predict=gmm_convert(x,$vectorLength,$mixNum{$emotion}[0],\'$trainDir/neutral_$emotion.phrase.gmm\',\'trajectory_d\');\n";
			print M "fprintf(fd,\'%f\\n\',predict);\n";
			print M "fprintf(fd,\'#\\n\');\n";	
=cut			
		}
		...
		print M "x=[@allphrase]';\n";
		print M "predict=gmm_convert(x,12,$mixNum{$emotion}[0],\'$trainDir/neutral_$emotion.phrase.gmm\',\'trajectory_d\');\n";
		for my $t(0..$T-1)
		{
			print M "fprintf(fd,\'$alltext[$t]\\n\');\n";
			print M "fprintf(fd,\'%f\\n\',predict($t*3+1:($t+1)*3));\n";
