...
#===============================================================================
#
#         FILE: twotier_nodynamic_diag.pl
#
#        USAGE: ./twotier_nodynamic_diag.pl  
#
#  DESCRIPTION: 
#
#      OPTIONS: ---
...
if($doTest)
{
	...
	for my $testPhraseFile(@testPhraseFiles)
	{
		...
		while(<PHRASE>)
		{
			...
			print M "predict=gmm_convert(x,$vectorLength,$mixNum{$emotion}[0],\'$trainDir/neutral_$emotion.phrase.gmm\',\'diag\');\n";
	...
	for my $testToneFile(@testToneFiles)
	{
		...
		print M "x=[@alltone]';\n";
		print M "predict=gmm_convert(x,16,$mixNum{$emotion}[1],\'$trainDir/neutral_$emotion.tone.gmm\',\'diag\');\n";
		for my $t(0..$T-1)
		{
