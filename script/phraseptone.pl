...
#         FILE: phraseptone.pl
#
#        USAGE: ./phraseptone.pl
#
#  DESCRIPTION: 特征提取及训练流程
#
#      OPTIONS: ---
# REQUIREMENTS: ---
#         BUGS: ---
...
my (
    $clean,           $emotion,  $init,
    $extractNeutral,  $extract1, $extract2,
    $prepareForTrain, $doTrain,  $doTest
   );
...
my $testDir =
  "$prjDir/test/twotier/$emotion/$mixNum{$emotion}[0]mix$mixNum{$emotion}[1]mix_$dctNumOfPhrase";
my $trainDir =
  "$prjDir/train/twotier/$emotion/$mixNum{$emotion}[0]mix$mixNum{$emotion}[1]mix_$dctNumOfPhrase";
...
if ($clean)
{
    system("rm -R $prjDir/$emotion/dct/twotier  $testDir $trainDir");
}
###################initialize#########################
if ($init)
{
    ...
    mkdir "$prjDir/train/twotier",          0755;
    mkdir "$prjDir/train/twotier/$emotion", 0755;
    mkdir "$trainDir",                      0755;
    mkdir "$prjDir/test",                   0755;
    mkdir "$prjDir/test/twotier",           0755;
    mkdir "$prjDir/test/twotier/$emotion",  0755;
    mkdir "$testDir",                       0755;
}
##################提取phrase dct########################
if ($extract1)
{
    for my $dir (@pdir)
    {
        open PB, "<$prjDir/neutral/forcedAlignment/newphrasebreak"
          or die "can't open";
        mkdir "$dir/dct/twotier", 0755;
        ...
        while (<PB>)
        {
            ...
            chomp @f0data;
            close F0;
            open PHRASE, ">$dir/dct/twotier/$base.phrase"
              or die "can't open $!\n";
            my ($pp, $cp, $np);
            $pp = 1;
            my $i = 1;
            ...
            open OUTF0, ">$dir/dct/twotier/$base.f0" or die "can't open $!\n";
            while ($line2 = <LAB>)
            {
...
if ($extract2)
{
    ...
    for my $dir (@pdir)
    {
        ...
        foreach (@files)
        {
            ...
            my $dctfile = $name . 'dct';
            open F0,   "<$dir/dct/twotier/$name.f0"   or die "can't open";
            open TONE, ">$dir/dct/twotier/$name.tone" or die "can't open";
            my $line;
            while ($line = <F0>)
            {
                ...
                if ($line =~ /[a-z]+/)
                {
                    ...
                    open TEMP, ">$dir/dct/twotier/temp" or die "can't open";
                    print TONE $line;
                    my $start;
                    my $end;
                    ...
                    my @dct =
                      `x2x +af $dir/dct/twotier/temp|dct -l $length|sopr -d sqrt$length|x2x +fa`;
                    ...
                    print TONE @dct;
                    print TONE "#\n";
                    system("rm -f $dir/twotier/dct/temp");
                }    #end if
            }    #end while
            close(TONE);
            close(F0);
...
if ($prepareForTrain)
{
    ...
    for my $paraKind (@paraKind)
    {
        ...
        for my $emotionKind (@emotions)
        {
            ...
            my @paraFiles = <$dir/dct/twotier/*.$paraKind>;
...
if ($doTrain)
{
    for my $paraKind ("phrase", "tone")
    {
        ...
        {
            system(
                "$SPTK/x2x +af $trainDir/$emotionKind.$paraKind >$trainDir/$emotionKind.$paraKind.f"
            );
        }
    }

    system(
        "$SPTK/merge -s 0 -l $dctNumOfPhrase -L $dctNumOfPhrase +f $trainDir/neutral.phrase.f $trainDir/$emotion.phrase.f>$trainDir/neutral_$emotion.phrase.f"
    );
    system("rm -f $trainDir/neutral.phrase.f $trainDir/$emotion.phrase.f");
    system(
        "$SPTK/merge -s 0 -l $dctNum -L $dctNum +f $trainDir/neutral.tone.f $trainDir/$emotion.tone.f>$trainDir/neutral_$emotion.tone.f"
    );
    system("rm -f $trainDir/neutral.tone.f $trainDir/$emotion.tone.f");
    my $vectorLength = 2 * $dctNumOfPhrase;
    system(
        "$SPTK/gmm -l $vectorLength -m $mixNum{$emotion}[0] -f $trainDir/neutral_$emotion.phrase.f >$trainDir/neutral_$emotion.phrase.gmm.f"
    );
    system(
        "$SPTK/gmm -l 16 -m $mixNum{$emotion}[1] -f $trainDir/neutral_$emotion.tone.f >$trainDir/neutral_$emotion.tone.gmm.f"
    );
    system(
        "$SPTK/x2x +fa $trainDir/neutral_$emotion.phrase.gmm.f>$trainDir/neutral_$emotion.phrase.gmm"
    );
    system(
        "$SPTK/x2x +fa $trainDir/neutral_$emotion.tone.gmm.f>$trainDir/neutral_$emotion.tone.gmm"
    );
...
if ($doTest)
{
    ...
    {
        system("cp $prjDir/neutral/dct/twotier/$iFile.phrase $testDir");
        system("cp $prjDir/neutral/dct/twotier/$iFile.tone $testDir");
    }
    ...
    for my $testPhraseFile (@testPhraseFiles)
    {
        ...
        while (<PHRASE>)
        {
            ...
            print M
              "predict=gmm_convert(x,$vectorLength,$mixNum{$emotion}[0],\'$trainDir/neutral_$emotion.phrase.gmm\');\n";
    ...
    for my $testToneFile (@testToneFiles)
    {
        ...
        while (<TONE>)
        {
            ...
            print M "x=[@toneParas]';\n";
            print M
              "predict=gmm_convert(x,16,$mixNum{$emotion}[1],\'$trainDir/neutral_$emotion.tone.gmm\');\n";
            print M "fprintf(fd,\'%f\\n\',predict);\n";
            print M "fprintf(fd,\'#\\n\');\n";
        }
        close TONE;
    ...
    for my $predictPhraseFile (@predictPhraseFiles)
    {
        ...
        while (<PHRASE>)
        {
            ...
            my @f0OfPhrase =
              `echo @phraseParas|$SPTK/x2x +af|$SPTK/bcut +f -s 0 -e $bcutEnd|$SPTK/idct -l $lenOfPhrase|$SPTK/sopr -m sqrt$lenOfPhrase|$SPTK/x2x +fa`;
            print TEMP "$text\n";
            print TEMP @f0OfPhrase;
            print TEMP "#\n";
        }
        close PHRASE;
        close TEMP;
