...
logfd=fopen([trainDir '/log_full2_order' num2str(order)],'w');
%% read gmm model
...
for iMix=1:mixNum
    ...
    for iMix=1:mixNum
        for iL=l/2+1:l
           ...
           for iO=1:order
               if abs(temp(iO))<1
                    ...
                    ar(iL,iO,iMix)=temp(iO)/(abs(temp(iO)));
               end
           end
       ...
       w(iMix)=accW(iMix)/numOfSeg;
             sigma(:,:,iMix)=accSig(:,:,iMix)./accW(iMix)-    accCur(:,iMix)./accW(iMix)*accCur(:,iMix)'./accW(iMix)  ;
     for iO=1:order  
           
            sigma(:,:,iMix)=sigma(:,:,iMix)-B(:,iO,iMix)*ar(:,iO,iMix)';
     end
...
%iteration
ite=1;
while(abs((curProb-preProb)/preProb)>1e-5 | ite>100)
liklihoodProb=0;
 numOfSeg=0;
A=zeros(l,order,order,mixNum);
B=zeros(l,order,mixNum);
