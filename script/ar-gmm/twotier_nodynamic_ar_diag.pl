...
#===============================================================================
#
#         FILE: towtier_nodynamic_ar_diag.pl
#
# 
#        USAGE: ./twotier_nodynamic_ar_diag.pl  ../config/config.pm  emotion mixNumOfPhrase mixNumOfTone mode
#
#  DESCRIPTION: 
#
#      OPTIONS: ---
...
my $testDir="$prjDir/test/$method"."_ar_diag"."/$emotion/$mixNum{$emotion}[0]mix$mixNum{$emotion}[1]mix_$dctNumOfPhrase";
...
if($init)
{
	...
	mkdir "$prjDir/test/$method"."_ar",0755;
	mkdir "$prjDir/test/$method"."_ar"."/$emotion",0755;
	mkdir "$testDir",0755;
}
...
if ($doTrain)
{
	...
	system("$SPTK/gmm -b 100 -l $vectorLength -m $mixNum{$emotion}[0]  $trainDir/neutral_$emotion.phrase.f >$trainDir/neutral_$emotion.phrase.gmm_diag.f");
	system("$SPTK/gmm -b 100 -l 16 -m $mixNum{$emotion}[1]  $trainDir/neutral_$emotion.tone.f >$trainDir/neutral_$emotion.tone.gmm_diag.f");
	system("$SPTK/x2x +fa $trainDir/neutral_$emotion.phrase.gmm_diag.f>$trainDir/neutral_$emotion.phrase.gmm_diag");
	system("$SPTK/x2x +fa $trainDir/neutral_$emotion.tone.gmm_diag.f>$trainDir/neutral_$emotion.tone.gmm_diag");
...
if($doResynth)
{
	...
	for my $testToneFile(@testToneFiles)
	{
		...
		while(<TONE>)
		{
			...
			print M "predict=gmm_convert(x,16,$mixNum{$emotion}[1],\'$trainDir/neutral_$emotion.tone.gmm\');\n";
            print M "fprintf(fd,\'%f\\n\',predict);\n";
            print M "fprintf(fd,\'#\\n\');\n";
