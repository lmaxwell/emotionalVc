...
B=zeros(order+1+l/2,order+1+l/2,l/2,mixNum);
D=zeros(mixNum,order+1+l/2,l/2);
mean=zeros(l,1);
...
for iSen=1:numOfSen
 ...
 %       fprintf(1,'%dth segment\n',iSeg);
        ...
        for iL=1:l/2
            s=[preSegs(iL,:) 1]';
            r=[preSegs(l/2+iL,:) 1 trainData(1:l/2,start+iSeg)' ]';
...
%%
    for iMix=1:mixNum
        ...
        for iL=1:l/2
            ...
            bt(iL,:,iMix)=temp(order+2:order+l/2+1);
               %bt(iL,iMix)=0;
        end
       ...
       w(iMix)=accW(iMix)/numOfSeg;
         sigmaxx(:,:,iMix)=diag(accSigx(:,iMix)-accSigx1(:,iMix).*miux(:,iMix))./accW(iMix);
          ...
          sigmayy(:,:,iMix)=diag(accSigy(:,iMix)-accSigy1(:,iMix).*miuy(:,iMix))./accW(iMix) -accSigy3(:,:,iMix)*bt(:,:,iMix)'./accW(iMix) ;
     ...
     if(prod(var(:,iMix))<=0)
             fprintf(1,'add variance floor %dmix\n',iMix);
...
%iteration
ite=1;
while(abs((curProb-preProb)/preProb)>1e-5 && ite <17)
A=zeros(order+1,order+1,l/2,mixNum);
C=zeros(mixNum,order+1,l/2);
B=zeros(order+1+l/2,order+1+l/2,l/2,mixNum);
D=zeros(mixNum,order+1+l/2,l/2);
...
%      end
   ...
   %       sigmayy(:,:,iMix)=diag(accSigy(:,iMix)-accSigy1(:,iMix).*miuy(:,iMix)  - bt(:,:,iMix) *accSigy3(:,iMix)         )./accW(iMix);
              sigmayy(:,:,iMix)=diag(accSigy(:,iMix)-accSigy1(:,iMix).*miuy(:,iMix))./accW(iMix) -accSigy3(:,:,iMix)*bt(:,:,iMix)'./accW(iMix)  ;
     for iO=1:order  
...
%%
ogmm=fopen([trainDir '/neutral_' emotion '.tone.gmm_ar_ABfull_order' num2str(order)],'w');
for iMix=1:mixNum
    fprintf(ogmm,'%f\n',w(iMix));
end
for iMix=1:mixNum
