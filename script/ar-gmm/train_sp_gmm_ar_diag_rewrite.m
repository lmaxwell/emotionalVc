...
btzero=1;
addpath('lightspeed');
logfd=fopen([trainDir '/log_xy_diag_order' num2str(ordery) ],'w');
%% read gmm model
...
A2=zeros(order+1,order+1,l/2,mixNum);
C2=zeros(mixNum,order+1,l/2);
B2=zeros(order+1,order+1,l/2,mixNum);
D2=zeros(mixNum,order+1,l/2);


accSigx2=zeros(l/2,mixNum);
accSigx12=zeros(l/2,mixNum);
accSigx22=zeros(l/2,mixNum,order);
accSigy2=zeros(l/2,mixNum);
accSigy12=zeros(l/2,mixNum);
accSigy22=zeros(l/2,mixNum,order);
accSigy32=zeros(l/2,mixNum);
    ...
    for iMix=1:mixNum
       ...
       w(iMix)=accW(iMix)/numOfSeg;
        ...
        %  sigmayy(:,:,iMix)=diag(accSig(l/2+1:l,iMix)-accSig1(l/2+1:l,iMix).*miuy(:,iMix)  -accSigy3(:,iMix).*bt(:,iMix)        )./accW(iMix);
          var(l/2+1:l,iMix)= ( accSig(l/2+1:l,iMix)-accSig1(l/2+1:l,iMix).*miuy(:,iMix)  )./accW(iMix) ;
     for iO=1:ordery  
