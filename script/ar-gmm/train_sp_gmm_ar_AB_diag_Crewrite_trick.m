...
btzero=1;
addpath('lightspeed');
logfd=fopen([trainDir '/log_xy_ABdiag_order' num2str(ordery) '_trick' ],'w');
%% read gmm model
...
A=zeros(mixNum,l/2,orderx,orderx);
C=zeros(mixNum,l/2,orderx);
B=zeros(mixNum,l/2,ordery+1,ordery+1);
D=zeros(mixNum,l/2,ordery+1);
            ...
            %     A(:,:,ordery+1,iOd)=A(:,:,ordery+1,iOd)+postProb*preSegs(1:l/2,iOd)' ;
              %    A(:,:,iOd,ordery+1)=A(:,:,iOd,ordery+1)+ postProb*preSegs(1:l/2,iOd)'; 
               % B(:,:,ordery+1,iOd)=B(:,:,ordery+1,iOd)+postProb*preSegs(l/2+1:l,iOd)' ;
                 B(:,:,ordery+1,iOd)=B(:,:,ordery+1,iOd)+postProb*( trainData(1:l/2,start+iSeg).*preSegs(l/2+1:l,iOd) )';
                  %B(:,:,iOd,ordery+1)=B(:,:,iOd,ordery+1)+ postProb*preSegs(l/2+1:l,iOd)'; 
                   B(:,:,iOd,ordery+1)=B(:,:,iOd,ordery+1)+ postProb*( trainData(1:l/2,start+iSeg).*preSegs(l/2+1:l,iOd) )';  
             ...
             %       accSig2(:,:,iOd)=accSig2(:,:,iOd)+postProb*( trainData(1:l,start+iSeg).*preSegs(1:l,iOd) )';
                    %  accSig2(:,l/2+1:l,iOd)=accSig2(:,l/2+1:l,iOd)+postProb*( trainData(l/2+1:l,start+iSeg).*preSegs(l/2+1:l,iOd) )';
                       ...
                       accPre(:,:,iOd)=accPre(:,:,iOd)+postProb*preSegs(1:l,iOd)';
         end
...
%B(:,:,ordery+2,ordery+1)=  accMiu(1:l/2,:)';
  B(:,:,ordery+1,ordery+1)=accSig(1:l/2,:)';
   D(:,:,ordery+1)=accSigy3';
   ...
   for iO=1:order
    for jO=1:order
        A(:,:,iO,jO)=A(:,:,iO,jO)./(accW*ones(1,l/2)) - accPre(:,1:l/2,iO).*accPre(:,1:l/2,jO)./(accW*ones(1,l/2))./(accW*ones(1,l/2));
        B(:,:,iO,jO)=B(:,:,iO,jO)./(accW*ones(1,l/2)) - accPre(:,l/2+1:l,iO).*accPre(:,l/2+1:l,jO)./(accW*ones(1,l/2))./(accW*ones(1,l/2));
    end
         C(:,:,iO)=C(:,:,iO)./(accW*ones(1,l/2)) - accMiu(1:l/2,:)'.*accPre(:,1:l/2,iO)./(accW*ones(1,l/2))./(accW*ones(1,l/2));
         D(:,:,iO)=D(:,:,iO)./(accW*ones(1,l/2))  - accMiu(l/2+1:l,:)'.*accPre(:,l/2+1:l,iO)./(accW*ones(1,l/2))./(accW*ones(1,l/2));
         B(:,:,iO,order+1)=B(:,:,iO,order+1)./(accW*ones(1,l/2)) - accMiu(1:l/2,:)'.*accPre(:,l/2+1:l,iO)./(accW*ones(1,l/2))./(accW*ones(1,l/2));
         B(:,:,order+1,iO)=B(:,:,order+1,iO)./(accW*ones(1,l/2)) - accMiu(1:l/2,:)'.*accPre(:,l/2+1:l,iO)./(accW*ones(1,l/2))./(accW*ones(1,l/2));
   end
     D(:,:,order+1)=D(:,:,order+1)./(accW*ones(1,l/2)) - accMiu(l/2+1:l,:)'.*accMiu(1:l/2,:)'./(accW*ones(1,l/2))./(accW*ones(1,l/2));
     %%
     B(:,:,order+1,order+1)=B(:,:,order+1,order+1)./(accW*ones(1,l/2)) - accMiu(1:l/2,:)'.*accMiu(1:l/2,:)'./(accW*ones(1,l/2))./(accW*ones(1,l/2));
%%
    for iMix=1:mixNum
               for iL=1:l/2
       ...
       % miux(:,iMix)=accMiu(1:l/2,iMix)./accW(iMix);
        for iL=1:l/2
         ...
         %   miuy(iL,iMix)=temp(ordery+1);
            ...
            bt(iL,iMix)=temp(ordery+1);
          
            %bt(iL,iMix)=0;
        end
        
        miux(:,iMix)=accMiu(1:l/2,iMix)./accW(iMix);
         miuy(:,iMix)=accMiu(l/2+1:l,iMix)./accW(iMix);
       ...
       w(iMix)=accW(iMix)/numOfSeg;
        ...
        %  sigmayy(:,:,iMix)=diag(accSig(l/2+1:l,iMix)-accSig1(l/2+1:l,iMix).*miuy(:,iMix)  -accSigy3(:,iMix).*bt(:,iMix)        )./accW(iMix);
          var(l/2+1:l,iMix)= ( accSig(l/2+1:l,iMix)-accSig1(l/2+1:l,iMix).*miuy(:,iMix)      )./accW(iMix)  - squeeze(D(iMix,:,order+1))'.*bt(:,iMix)   ;
     for iO=1:ordery  
        var(1:l/2,iMix)=var(1:l/2,iMix)-squeeze(C(iMix,:,iO))'.*arx(:,iO,iMix);
          %  sigmaxx(:,:,iMix)=sigmaxx(:,:,iMix)-diag(accSig2(1:l/2,iO,iMix).*arx(:,iO,iMix) )./accW(iMix);
           % sigmayy(:,:,iMix)=sigmayy(:,:,iMix)-diag( accSig2(iMix,l/2+1:l,iO)'.*ary(:,iO,iMix)  )./accW(iMix);
            var(l/2+1:l,iMix)=var(l/2+1:l,iMix)-squeeze(D(iMix,:,iO))'.*ary(:,iO,iMix) ;
     end
...
%         end
        ...
        curMiu=[miux ;miuy];
        for iO=1:ordery
            curMiu(1:l/2,:)=curMiu(1:l/2,:)+ (preSegs(1:l/2,iO)*ones(1,mixNum)-squeeze(preMiu(:,1:l/2,iO))' ).*squeeze(arx(:,iO,:));
            curMiu(l/2+1:l,:)=curMiu(l/2+1:l,:) + (preSegs(l/2+1:l,iO)*ones(1,mixNum)-squeeze(preMiu(:,l/2+1:l,iO))' ).*squeeze( ary(:,iO,:) );
        end
        
        curMiu(l/2+1:l,:)=curMiu(l/2+1:l,:) + (trainData(1:l/2,start+iSeg)*ones(1,mixNum)-miux).*bt;
...
ogmm=fopen([trainDir '/../../gmm_para/'  num2str(nMix) '-mix' '/neutral-' emotion num2str(nMix) '_ordery' num2str(ordery)  '_trick'  '.para.txt' ],'w');
for iMix=1:mixNum
    fprintf(ogmm,'%f\n',w(iMix));
end
for iMix=1:mixNum
