...
my $testDir="$prjDir/test/$method"."_ar_full"."/$emotion/$mixNum{$emotion}[0]mix$mixNum{$emotion}[1]mix_$dctNumOfPhrase";
...
if($init)
{
	...
	mkdir "$prjDir/test/$method"."_ar_full",0755;
	mkdir "$prjDir/test/$method"."_ar_full"."/$emotion",0755;
	mkdir "$testDir",0755;
}
...
if($doResynth)
{
	...
	for my $testToneFile(@testToneFiles)
	{
		...
		print M "x=[@alltone]';\n";
		print M "predict=gmm_ar_convert(x,16,1,$mixNum{$emotion}[1],\'$trainDir/neutral_$emotion.tone.gmm_ar_full\',\'full\');\n";
		for my $t(0..$T-1)
		{
			print M "fprintf(fd,\'$alltext[$t]\\n\');\n";
			print M "fprintf(fd,\'%f\\n\',predict($t*8+1:($t+1)*8));\n";
