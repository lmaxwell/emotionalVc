...
order=1;
addpath('lightspeed');
logfd=fopen([trainDir '/log_xy_ABdiag_order' num2str(ordery) '_onlysrc' num2str(btzero)],'w');
%% read gmm model
...
% %             end
%        end
        postProb=gconst- ndsum(0.5.*(trainData(1:l/2,start+iSeg)*ones(1,mixNum)-miu(1:l/2,:)).*(trainData(1:l/2,start+iSeg)*ones(1,mixNum)-miu(1:l/2,:))./var(1:l/2,:), 1) ;
