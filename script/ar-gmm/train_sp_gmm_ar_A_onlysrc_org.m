...
order=1;
addpath('lightspeed');
logfd=fopen([trainDir '/log_xy_ABdiag_order' num2str(ordery) '_onlytrg_org' num2str(btzero)],'w');
%% read gmm model
...
for iMix=1:mixNum
   ...
   var(:,iMix)=diag( sigma(:,:,iMix) );
    ...
    gconst(iMix)=log(w(iMix))-l/2/2*log(2*pi)-0.5*log(prod( var(l/2+1:l,iMix) ));
...
%     
%    var(:,iMix)=diag( sigma(:,:,iMix) );
%   gconst(iMix)=log(w(iMix))-l/2*log(2*pi)-0.5*log(prod( var(1:l/2,iMix) ));
%     
%  
% end 
% bt=zeros(l/2,mixNum);
...
% %             end
%        end
        postProb=gconst- ndsum(0.5.*(trainData(1:l/2,start+iSeg)*ones(1,mixNum)-miu(l/2+1:l,:)).*(trainData(1:l/2,start+iSeg)*ones(1,mixNum)-miu(l/2+1:l,:))./var(l/2+1:l,:), 1) ;
          ...
          %          B(:,:,iOd,jOd)=B(:,:,iOd,jOd)+postProb* ( preSegs(l/2+1:l,iOd).*preSegs(l/2+1:l,jOd) )'; 
           ...
           %      B(:,:,ordery+2,iOd)=B(:,:,ordery+2,iOd)+postProb*( trainData(1:l/2,start+iSeg).*preSegs(l/2+1:l,iOd) )';
             ...
             %      B(:,:,iOd,ordery+2)=B(:,:,iOd,ordery+2)+ postProb*( trainData(1:l/2,start+iSeg).*preSegs(l/2+1:l,iOd) )';  
                    ...
                    accSig2(:,1:l/2,iOd)=accSig2(:,1:l/2,iOd)+postProb*( trainData(1:l/2,start+iSeg).*preSegs(1:l/2,iOd) )';
      ...
      % B(:,:,ordery+1,ordery+1)=   B(:,:,ordery+1,ordery+1)+postProb*ones(1,l/2);  
       ...
       %   B(:,:,ordery+1,ordery+2)=   B(:,:,ordery+1,ordery+2)+postProb*trainData(1:l/2,start+iSeg)';  
         ...
         %     D(:,:,ordery+2)=D(:,:,ordery+2)+postProb*( trainData(l/2+1:l,start+iSeg).*trainData(1:l/2,start+iSeg) )'; 
                   ...
                   accSig(1:l/2,:)=accSig(1:l/2,:)+trainData(1:l/2,start+iSeg).*trainData(1:l/2,start+iSeg)*postProb';
         ...
         accMiu(1:l/2,:)=accMiu(1:l/2,:)+trainData(1:l/2,start+iSeg)*postProb' ;     %   =B(:,:,ordery+1,ordery+2) = B(:,:,ordery+2,ordery+1)
        ...
        vFloor=vFloor+(trainData(:,start+iSeg)-miuy(:,maxMix)).* (trainData(:,start+iSeg)-miuy(:,maxMix));
...
%       end
        ...
        gconst(iMix)=log(w(iMix))-l/2/2*log(2*pi)-0.5*log(prod( var(1:l/2,iMix) ));
  %  invSigma(:,:,iMix)=inv( sigma(:,:,iMix) );
...
%         end
        ...
        postProb=gconst- ndsum(0.5.*(trainData(1:l/2,start+iSeg)*ones(1,mixNum)-curMiu(1:l/2,:)).*(trainData(1:l/2,start+iSeg)*ones(1,mixNum)-curMiu(1:l/2,:))./var(1:l/2,:), 1) ;
         sum_postProb=logsumexp(postProb);
        postProb=exp(postProb-sum_postProb);
        accW=accW+postProb;
        liklihoodProb=liklihoodProb+sum_postProb;
