...
void readModel(FILE *fgmm,GMM *gmm)
{
	...
	//	printf("iO=%d,m=%d\n",iO,m);
		  pp=gmm->gauss[m].ar.data;
			memcpy(*(pp+iO),data+pos,sizeof(float)*L/2);
			pos+=L/2;
		}
		for(iO=0;iO<order;iO++)
		{
	...
	//	printf("iO=%d,m=%d,pos=%d\n",iO,m,pos);
		...
		memcpy(gmm->gauss[m].bt.data,data+pos,sizeof(float)*L/2);
		pos+=L/2;
...
{
	int i,m,iO,L=gmm->L,M=gmm->M,order=gmm->order,maxMix=0;
	...
	for(i=0;i<L/2;i++)
	{
		...
		conv[i]+=gmm->gauss[maxMix].bt.data[i]*src[i];
	}
}
...
int main()
{
  ...
  pre.col=48;
	...
	FILE *fcmcep=fopen("201.cmcep","wb");
	for (iT=0;iT<T;iT++)
	{
